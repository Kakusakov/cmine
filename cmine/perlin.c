...
PerlinSeed* perlin_init_from_stdrand(Arena* arena, unsigned rand_seed) {
	PerlinSeed* seed = arena_alloc(arena, 1, PerlinSeed);
	...
	return (lerp(w,
		lerp(v,
			lerp(u, 
				...
				grad3(seed->p[ba], x - 1.0f, y, z)
			),
			lerp(u, 
				grad3(seed->p[ab], x, y - 1, z),
				grad3(seed->p[bb], x - 1.0f, y - 1.0f, z)
			)
		),
		lerp(v,
			lerp(u, 
				grad3(seed->p[aa + 1], x, y, z - 1.0f),
				grad3(seed->p[ba + 1], x - 1.0f, y, z - 1.0f)
			),
			lerp(u, 
				grad3(seed->p[ab + 1], x, y - 1.0f, z - 1.0f),
				grad3(seed->p[bb + 1], x - 1.0f, y - 1.0f, z - 1.0f)
			)
		)
	) + 1.0f) / 2.0f;
}
...
float32_t perlin2(const PerlinSeed* seed, float32_t x, float32_t y) {
	...
	return (lerp(v,
		lerp(u, 
			grad2(seed->p[a], x, y),
			grad2(seed->p[b], x - 1.0f, y)
		),
		lerp(u, 
			grad2(seed->p[a + 1], x, y - 1.0f),
			grad2(seed->p[b + 1], x - 1.0f, y - 1.0f)
		)
	) + 1.0f) / 2.0f;
}
...
float32_t fbm3(
	...
	float32_t frequency = settings->frequency;
	float32_t intensity = settings->intensity;
